    ...
    <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left py-3 px-4 font-medium text-gray-500 text-sm">品目</th>
                <th className="text-left py-3 px-4 font-medium text-gray-500 text-sm">目標在庫</th>
                <th className="text-left py-3 px-4 font-medium text-gray-500 text-sm">栽培日数</th>
                <th className="text-left py-3 px-4 font-medium text-gray-500 text-sm">単価</th>
              </tr>
            </thead>
            <tbody>
          ...
          {productionSettings.map((product) => (
            <div key={product.id} className="bg-gray-50 rounded-xl p-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-gray-500 mb-1">目標在庫</label>
                  <Input
                    type="number"
                    value={product.targetInventory}
                    onChange={(e) => {
                ...
                <div>
                  <label className="block text-xs text-gray-500 mb-1">栽培日数</label>
                  <p className="h-9 flex items-center text-gray-600">{product.growthDays}日</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="rounded-2xl bg-white border border-gray-100 p-4 sm:p-6 shadow-sm">
        ...
        <div className="space-y-1">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between py-3 border-b border-gray-100 gap-2">
            <div>
              <p className="font-medium text-gray-900 text-sm sm:text-base">在庫有効期限</p>
              <p className="text-xs sm:text-sm text-gray-500">この日数を過ぎると廃棄対象</p>
            </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between py-3 border-b border-gray-100 gap-2">
            <div>
              <p className="font-medium text-gray-900 text-sm sm:text-base">最小発注リードタイム</p>
              <p className="text-xs sm:text-sm text-gray-500">発注から納品までの最小日数</p>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between py-3 gap-2">
            <div>
              <p className="font-medium text-gray-900 text-sm sm:text-base">データバックアップ</p>
              <p className="text-xs sm:text-sm text-gray-500">最終バックアップ: 2024/12/16 03:00</p>
            </div>
            <Button variant="outline" className="w-full sm:w-auto">今すぐバックアップ</Button>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl bg-white border border-gray-100 p-4 sm:p-6 shadow-sm">
        ...
        <div className="space-y-1">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between py-3 border-b border-gray-100 gap-2">
            <div>
              <p className="font-medium text-gray-900 text-sm sm:text-base">二要素認証</p>
              <p className="text-xs sm:text-sm text-gray-500">より安全なログインのために推奨</p>
            </div>
            <Button variant="outline" className="w-full sm:w-auto">設定する</Button>
          </div>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between py-3 gap-2">
            <div>
              <p className="font-medium text-gray-900 text-sm sm:text-base">ログイン履歴</p>
              <p className="text-xs sm:text-sm text-gray-500">最近のログイン活動を確認</p>
            </div>
            <Button variant="outline" className="w-full sm:w-auto">履歴を見る</Button>
          </div>
        </div>
      </div>
